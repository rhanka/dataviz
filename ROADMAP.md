...
- [x] Couche analytique (réf lines, bandes/percentiles, tendance, prévision, cluster, barres d'erreur, goal line) — contrats core ref/percentile/trend/forecast/cluster/error bars faits ; wrappers ReferenceLine/PercentileBand/TrendLine/ForecastLine/ErrorBars/AnalyticsCluster livrés en Svelte/React/Vue. Les SVG fallbacks v0.4.16/v0.4.17 sont une couverture fonctionnelle temporaire, à remplacer par surfaces DS via WP15.
...
- [ ] Analytics : remplacer les fallbacks dataviz par surfaces DS. Correction dataviz immédiate à étudier pour `ReferenceLineChart`, `TrendLineChart` et `ErrorBarsChart` via les `LineChart`/`BarChart` DS existants ; handoff DS minimal requis pour bande percentile compacte + médiane, forecast pointillé, scatter/cluster avec rayon par datum.
...
- [ ] Versions DS : repinner dataviz sur les versions DS courantes après remplacement des fallbacks (`@sentropic/design-system-react`/`vue` 0.26.1, `svelte` 0.30.1 ou version ultérieure publiée).
