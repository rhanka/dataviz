...
- [x] Couche analytique (réf lines, bandes/percentiles, tendance, prévision, cluster, barres d'erreur, goal line) — contrats core ref/percentile/trend/forecast/cluster/error bars faits ; wrappers ReferenceLine/PercentileBand/TrendLine/ForecastLine/ErrorBars/AnalyticsCluster livrés en Svelte/React/Vue (SVG fallback pour overlays non exposés côté DS)
- [x] Cartographie géo (pin, choroplèthe/filled, densité, GeoJSON/shape, flux/arcs, hexbin, clustering/couches ; 3D hors-DS) — contrats core pin/choropleth/densité/GeoJSON/flow/hexbin/cluster faits ; wrappers GeoPoint/Choropleth/GeoFlow/GeoHexbin/GeoCluster/GeoDensity/GeoJson livrés en Svelte/React/Vue (SVG fallback pour cartes/couches non exposées côté DS)
