...
- [x] Couche analytique (réf lines, bandes/percentiles, tendance, prévision, cluster, barres d'erreur, goal line) — contrats core ref/percentile/trend/forecast/cluster/error bars faits ; wrappers ReferenceLine/PercentileBand/TrendLine/ForecastLine/ErrorBars/AnalyticsCluster livrés en Svelte/React/Vue. WP15 : ReferenceLine/PercentileBand/TrendLine/ErrorBars sont revenus sur les surfaces DS LineChart/BarChart en 3 frameworks ; ForecastLine et AnalyticsCluster restent des fallbacks temporaires en attente de surfaces DS dédiées.
- [x] Cartographie géo (pin, choroplèthe/filled, densité, GeoJSON/shape, flux/arcs, hexbin, clustering/couches ; 3D hors-DS) — contrats core pin/choropleth/densité/GeoJSON/flow/hexbin/cluster faits ; wrappers GeoPoint/Choropleth/GeoFlow/GeoHexbin/GeoCluster/GeoDensity/GeoJson livrés en Svelte/React/Vue. Les SVG fallbacks v0.4.17 sont une couverture fonctionnelle temporaire, à remplacer par surfaces DS via WP15.
...
- [x] Analytics / correction dataviz immédiate : `ReferenceLineChart`, `PercentileBandChart`, `TrendLineChart` et `ErrorBarsChart` utilisent les surfaces DS existantes (`LineChart`/`BarChart`) en Svelte/React/Vue ; tests analytics verrouillés sur les classes DS.
- [ ] Analytics / handoff DS restant : remplacer les derniers fallbacks `ForecastLineChart` (ligne forecast pointillée native DS) et `AnalyticsClusterPlot` (scatter/cluster avec rayon par datum + centroïdes) ; détail dans `docs/audit/wp15-ds-handoff.md`.
- [ ] Geo : remplacer les 7 fallbacks dataviz (`GeoPointMap`, `ChoroplethMap`, `GeoFlowMap`, `GeoHexbinMap`, `GeoClusterMap`, `GeoDensityMap`, `GeoJsonMap`) par une surface DS carto 2D ou des composants DS dédiés à parité Svelte/React/Vue.
- [x] Versions DS : dataviz packages + apps démo repinnés sur `@sentropic/design-system-react`/`vue` 0.26.1 et `@sentropic/design-system-svelte` 0.30.1.
