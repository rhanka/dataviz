...
3. **QA finalités (bugs visuels récurrents)** 🟡 FR-DS majoritaire. (a) rendus « moches » / défauts de style par défaut ; (b) **chevauchement & texte coupé** (labels d'axes/légendes/data-labels → collision/rotation/ellipsis/every-nth) ; (c) **« trop gros »** / sizing & responsive (ratio, min/max, overflow clip) ; (d) **interactions cassées** (ex **filter range** ne filtre pas) ; (e) **animations qui n'ont pas de sens**. → grille de QA par chart (cf research) + FR DS pour les défauts de composant.
